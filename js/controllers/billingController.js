...
class BillingController {
  ...
  setupEventListeners() {
    ...
    const discountInput = document.querySelector('input[value="0"]');
    if (discountInput) {
    ...
    if (lifetimeDiscountCheckbox && discountInputEl) {
      lifetimeDiscountCheckbox.addEventListener("change", (e) => {
        ...
        if (checked) {
          ...
          this.updateBillTotals(parseFloat(discountInputEl.value) || 0);
        }
      });
    }

    // View Bills button and F10 shortcut
    const viewBillsBtn = document.getElementById("view-bills-btn");
    if (viewBillsBtn) {
      viewBillsBtn.addEventListener("click", () => this.focusRecentBillsSearch());
    }
    document.addEventListener("keydown", (e) => {
      if (e.key === "F10") {
        e.preventDefault();
        this.focusRecentBillsSearch();
      }
    });
  ...
  displayPatientHistory(history) {
    ...
    if (history.length === 0) {
      historyTable.innerHTML = `
            ...
            `;
      this.updateCustomerOutstanding(0);
      return;
    }
    ...
    const totalOutstanding = history.reduce(
      (sum, bill) => sum + (parseFloat(bill.remaining_amount) || 0),
      0
    );
    this.updateCustomerOutstanding(totalOutstanding);
  }

  updateCustomerOutstanding(amount) {
    const outstandingEl = document.getElementById("customer-outstanding");
    if (outstandingEl) {
      outstandingEl.textContent = `Customer Outstanding: ${window.app.formatCurrency(amount)}`;
    }
  }
  ...
  getTestNames(billItems) {
    ...
    return billItems
      .map((item) => {
        if (item.tests)
          return item.tests.short_name || item.tests.test_name;
  ...
  autoFillPatientDetails(bill) {
    ...
    if (lifetimeDiscountCheckbox && discountInputEl) {
      ...
      if (isLifetime) {
        ...
        discountInputEl.removeAttribute("disabled");
        // Leave any manually typed discount as-is, but ensure totals recalc
        const val = parseFloat(discountInputEl.value) || 0;
        this.updateBillTotals(val);
      }
    }
  ...
  clearPatientHistory() {
    const historyTable =
      document.querySelector("#patient-history-table tbody") ||
      document.querySelectorAll(".table-responsive .table tbody")[1];
    if (historyTable) {
      historyTable.innerHTML = `
                <tr>
                    <td colspan="7" class="empty-state">
  ...
  handleDiscountChange(event) {
    ...
    this.updateBillTotals(discountPercentage);
  }
  ...
  // Update bill totals
  updateBillTotals(discountPercentage = 0) {
    const totalAmount = this.calculateTotalAmount();
    const discountAmount =
      (totalAmount * (parseFloat(discountPercentage) || 0)) / 100;
  ...
  resetBillForm() {
    ...
    // Clear bill items table and in-memory selections
    this.selectedTests = [];
    this.selectedPackages = [];
  ...
  focusRecentBillsSearch() {
    const recentBillsSearch = document.querySelector(
      'input[placeholder*="Search by Bill No or Patient Name"]'
    );
    if (recentBillsSearch) {
      recentBillsSearch.scrollIntoView({ behavior: "smooth", block: "center" });
      recentBillsSearch.focus();
      recentBillsSearch.select();
    }
  }
  ...
  displayBillDetails(bill) {
    let modal = document.getElementById("bill-details-modal");
    if (!modal) {
      document.body.insertAdjacentHTML(
        "beforeend",
        `<div class="modal fade" id="bill-details-modal" tabindex="-1" aria-hidden="true">
          <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
              ...
              <div class="modal-body" id="bill-details-body"></div>
      ...
      modal = document.getElementById("bill-details-modal");
    }

    const body = document.getElementById("bill-details-body");
    const items = (bill.bill_items || [])
      .map((item) => {
        if (item.tests) {
          return `<tr><td>${item.tests.short_name || item.tests.test_name}</td><td>${window.app.formatCurrency(item.total_price || item.tests.price || 0)}</td></tr>`;
        }
        if (item.packages) {
          return `<tr><td>${item.packages.package_name}</td><td>${window.app.formatCurrency(item.total_price || item.packages.price || 0)}</td></tr>`;
        }
        return "";
      })
      .join("");
    ...
    body.innerHTML = `
      <div class="row g-3 mb-3">
        <div class="col-md-6"><strong>Bill No:</strong> ${bill.bill_no}</div>
        <div class="col-md-6"><strong>Date:</strong> ${window.app.formatDate(bill.bill_date)}</div>
        <div class="col-md-6"><strong>Patient:</strong> ${bill.patient_title ? bill.patient_title + " " : ""}${bill.patient_name || ""}</div>
        <div class="col-md-6"><strong>Phone:</strong> ${bill.patient_phone || "N/A"}</div>
        <div class="col-md-6"><strong>Ref By:</strong> ${bill.ref_by || "N/A"}</div>
        <div class="col-md-6"><strong>Status:</strong> ${bill.status || "N/A"}</div>
      </div>
      <table class="table table-sm table-bordered">
        <thead><tr><th>Test</th><th>Amount</th></tr></thead>
        <tbody>${items || '<tr><td colspan="2">No items</td></tr>'}</tbody>
      </table>
      <div class="d-flex justify-content-between"><span>Total:</span><strong>${window.app.formatCurrency(bill.total_amount || 0)}</strong></div>
      <div class="d-flex justify-content-between"><span>Discount (${bill.discount || 0}%):</span><strong>${window.app.formatCurrency((bill.total_amount || 0) - (bill.final_amount || 0))}</strong></div>
      <div class="d-flex justify-content-between"><span>Final Amount:</span><strong>${window.app.formatCurrency(bill.final_amount || 0)}</strong></div>
      <div class="d-flex justify-content-between"><span>Paid:</span><strong>${window.app.formatCurrency(bill.paid_amount || 0)}</strong></div>
      <div class="d-flex justify-content-between"><span>Remaining:</span><strong>${window.app.formatCurrency(bill.remaining_amount || 0)}</strong></div>
    `;

    const title = modal.querySelector(".modal-title");
    if (title) title.textContent = `Bill ${bill.bill_no}`;
  ...
  populateFormWithBillData(bill) {
    ...
    this.updateBillTotals(
      bill.discount !== undefined && bill.discount !== null ? bill.discount : 0
    );
  ...
  async loadBillItemsForEditing(billId) {
    try {
      ...
      if (billItems && billItems.length > 0) {
        ...
        const currentDiscount = parseFloat(discountEl?.value) || 0;
        this.updateBillTotals(currentDiscount);
  ...
  updateTestTable() {
    ...
    if (!this.selectedTests || this.selectedTests.length === 0) {
      testTable.innerHTML = `<tr><td colspan="6" class="empty-state"><i class="fas fa-info-circle"></i><div>No tests added yet. Type a test name or short name in the search box above.</div></td></tr>`;
      return;
    }
  ...
  generateBillHTML(bill, billData, selectedTests, copyType) {
    ...
    const discountPercentage =
      bill.discount != null
        ...
        : parseFloat(document.getElementById("discount-amount")?.value) || 0;
    const discountAmount =
      bill.final_amount != null && bill.total_amount != null
        ? parseFloat(bill.total_amount) - parseFloat(bill.final_amount)
        : (totalAmount * discountPercentage) / 100;
    const finalAmount =
      bill.final_amount != null
        ? parseFloat(bill.final_amount)
    ...
    const singleBillContent = `
      ...
      <div class="header">
        <div class="logo-container">
          <img src="Imgs/suwa_sahana_logo.png" alt="SUWA SAHANA MADICAL CLINIC Logo" style="max-width: 100px; max-height: 100px; margin-bottom: 10px;" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
          <div class="logo-placeholder" style="display: none;">//LOGO HERE</div>
        </div>
        <div class="lab-name">SUWA SAHANA MADICAL CLINIC</div>
        <div class="lab-address">No 4/D, Colombo Road, Kiribathkumbura</div>
        <div class="lab-address">075 906 4328</div>
        ${
          copyType === "LAB COPY"
      ...
      <div class="tests-section">
        ${selectedTests
          .map((test) => {
            const displayName = test.short_name || test.test_name || "";
            const price = parseFloat(test.price) || 0;
            return `
          <div class="test-row">
            <span class="test-name">${displayName}</span>
            <span class="test-price">Rs. ${price.toFixed(2)}</span>
          </div>
        `;
          })
          .join("")}
      ...
      <div class="footer">
        ...
        <div class="footer-text">
          ...
          <div class="thank-you-text">Thank you for choosing SUWA SAHANA MADICAL CLINIC!</div>
