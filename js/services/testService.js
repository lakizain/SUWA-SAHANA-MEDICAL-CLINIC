...
class TestService {
  ...
  async createTest(testData) {
    ...
    try {
      ...
      if (
        ...
        (await window.SchemaHelper?.hasColumn(
          "tests",
          "footer_text",
          this.supabase
        ))
      ) {
        testPayload.footer_text = testData.footer_text;
      }
    ...
    try {
      ...
      if (name === "") {
        ...
        if (existingList && existingList.length > 0) {
          ...
          const hasUom = await window.SchemaHelper?.hasColumn(
            "test_subcategories",
            "uom",
            this.supabase
          );
          if (hasUom && uom && existing.uom !== uom) {
      ...
      if (hasUom) {
        subcategoryPayload.uom = uom || null;
        subcategoryPayload.less_more = extra?.less_more ?? null;
        subcategoryPayload.less_more_comment =
          extra?.less_more_comment ?? null;
        subcategoryPayload.less_more_mark = extra?.less_more_mark ?? null;
      }
  ...
  async updateTestSubcategory(subcategoryId, updateData) {
    try {
      ...
      if (hasUom) {
        if (updateData.uom !== undefined) payload.uom = updateData.uom;
        if (updateData.less_more !== undefined)
          payload.less_more = updateData.less_more;
        if (updateData.less_more_comment !== undefined)
          payload.less_more_comment = updateData.less_more_comment;
        if (updateData.less_more_mark !== undefined)
          payload.less_more_mark = updateData.less_more_mark;
      }
