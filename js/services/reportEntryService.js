...
class ReportEntryService {
  constructor() {
    window.bindSupabaseService?.(this);
  ...
  async getTestsSelectFields() {
    const base = "test_name, short_name, price, duration";
    const hasFooter = await window.SchemaHelper?.hasColumn(
      "tests",
      "footer_text",
      this.supabase
    );
    return hasFooter ? `${base}, footer_text` : base;
  }

  async billItemsWithTestsSelect() {
    const testsFields = await this.getTestsSelectFields();
    return `
          *,
          bill_items (
            *,
  ...
  async getTodaysBills() {
    try {
      ...
      const { data: bills, error } = await this.supabase
        .from("bills")
        .select(await this.billItemsWithTestsSelect())
  ...
  async getTestResultsByBillItem(billItemId) {
    try {
      ...
      const hasBillItemId = await window.SchemaHelper?.hasColumn(
        "test_results",
        "bill_item_id",
        this.supabase
      );
      ...
      if (hasBillItemId) {
        query = query.eq("bill_item_id", billItemId);
      } else {
        const { data: billItem, error: itemError } = await this.supabase
          .from("bill_items")
        ...
        const { data: subcategories, error: subError } = await this.supabase
          ...
          .select("id")
          .eq("test_id", billItem.test_id);
        if (subError) throw subError;

        const subcategoryIds = (subcategories || []).map((s) => s.id);
        query = query.eq("bill_id", billItem.bill_id);
      ...
      if (error) {
        const friendly = error?.message?.includes("does not exist")
          ? "Table test_results not found. Run database/SUWA SAHANA MADICAL CLINIC.sql in Supabase."
    ...
    try {
      ...
      const sanitizedResults = Array.isArray(results)
        ? results
            ...
            .map((r) => {
              ...
              if (hasBillItemId) {
                row.bill_item_id = billItemId;
              }
              return row;
            })
        : [];

      let deleteQuery = this.supabase.from("test_results").delete();
      if (hasBillItemId) {
        deleteQuery = deleteQuery.eq("bill_item_id", billItemId);
      } else {
        const subcategoryIds = sanitizedResults.map((r) => r.subcategory_id);
        deleteQuery = deleteQuery.eq("bill_id", billId);
        if (subcategoryIds.length > 0) {
          deleteQuery = deleteQuery.in("subcategory_id", subcategoryIds);
        }
      }
      ...
      if (delError) {
        ...
        if (msg.includes("does not exist")) {
          throw new Error(
            "test_results table not found. Run database/migration_add_missing_columns.sql in Supabase."
          );
        }
