...
class BillingService {
  ...
  async createBill(billData, retryCount = 0) {
    ...
    try {
      ...
      if (
        ...
        billPayload.lifetime_discount = true;
      }
      ...
      if (
        error &&
        billPayload.lifetime_discount !== undefined &&
        window.SchemaHelper?._isMissingColumnError?.(error, "lifetime_discount")
      ) {
        delete billPayload.lifetime_discount;
        ({ data: bill, error } = await this.supabase
          .from("bills")
          .insert(billPayload)
          .select()
          .single());
  ...
  async getPatientHistory(phone) {
    if (!this.isSupabaseAvailable()) {
      console.warn("Supabase not available, returning empty patient history");
      return [];
    }

    try {
      const includeLifetime = await window.SchemaHelper?.hasColumn(
        "bills",
        "lifetime_discount",
        this.supabase
      );
      const lifetimeField = includeLifetime ? "lifetime_discount,\n" : "";
  ...
  async updateBill(billId, updateData) {
    try {
      ...
      if (
        payload.lifetime_discount !== undefined &&
        !(await window.SchemaHelper?.hasColumn(
          "bills",
          "lifetime_discount",
          this.supabase
        ...
        delete payload.lifetime_discount;
      }
