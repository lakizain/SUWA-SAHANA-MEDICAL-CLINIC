-- SUWA SAHANA MADICAL CLINIC - COMPLETE DATABASE SCHEMA
-- Supabase SQL Editor: copy and run this entire script in one go.
-- Safe to re-run: uses IF NOT EXISTS, DROP TRIGGER IF EXISTS, and ON CONFLICT.
...
CREATE TABLE IF NOT EXISTS tests (
    ...
    CONSTRAINT check_footer_image_url CHECK (
        footer_image_url IS NULL OR footer_image_url ~* '^https?://'
    )
);
...
CREATE TABLE IF NOT EXISTS test_subcategories (
    ...
    uom VARCHAR(50),
    less_more TEXT CHECK (less_more IS NULL OR less_more IN ('less', 'more')),
...
DROP TRIGGER IF EXISTS update_test_report_headers_updated_at ON test_report_headers;
...
DROP TRIGGER IF EXISTS update_bill_totals_trigger ON bill_items;
...
DECLARE
    tbl TEXT;
    tables TEXT[] := ARRAY[
        'users', 'admin_users', 'centers', 'references', 'tests', 'packages',
        'bills', 'bill_items', 'test_results', 'test_report_headers',
        'test_subcategories', 'subcategory_suggestions', 'reference_ranges', 'package_tests'
    ];
        ...
        FOREACH r IN ARRAY roles LOOP
            IF NOT EXISTS (
                ...
                WHERE schemaname = 'public'
                  AND tablename = tbl
                  AND policyname = 'Allow all for ' || r
            ) THEN
                EXECUTE format(
...
('CID001', 'SUWA SAHANA MADICAL CLINIC', 'No 4/D, Colombo Road, Kiribathkumbura', '075 906 4328')
...
INSERT INTO tests (test_name, short_name, price, duration, test_cost, category, specimen, tube)
SELECT v.test_name, v.short_name, v.price, v.duration, v.test_cost, v.category, v.specimen, v.tube
FROM (VALUES
...
) AS v(test_name, short_name, price, duration, test_cost, category, specimen, tube)
WHERE NOT EXISTS (
    SELECT 1 FROM tests t WHERE t.short_name = v.short_name
);
...
('PGID002', 'BASIC HEALTH CHECK UP', 2100.00, 'Essential health screening package'),
('PGID003', 'PHI PACK', 2000.00, 'Basic health screening package'),
('PGID004', 'ADVANCED DIAGNOSTIC PACK', 3000.00, 'Advanced diagnostic package')
ON CONFLICT (pgid) DO NOTHING;

INSERT INTO admin_users (username, email, password, role) VALUES
('admin', 'admin@suwasahana.com', 'admin123', 'admin'),
('staff1', 'staff1@suwasahana.com', 'staff123', 'staff'),
('staff2', 'staff2@suwasahana.com', 'staff123', 'staff')
...
COMMENT ON TABLE centers IS 'Clinic centers/branches';
COMMENT ON TABLE "references" IS 'Medical references/doctors';
COMMENT ON TABLE tests IS 'Clinic tests and procedures';
...
-- (also available as database/migration_add_missing_columns.sql)
-- =============================================
